...
const SUPPORTED_TARGETS = new Set(["generic", "codex", "claude", "openclaw"]);
const DEFAULT_API_BASE_URL = "http://127.0.0.1:8787";
const MODULE_DIR = dirname(fileURLToPath(import.meta.url));
const PROJECT_DIR = join(MODULE_DIR, "..", "..");
const CANONICAL_SKILL_PATH = join(PROJECT_DIR, "skills", "agentcart-shopping-skill.md");

function assertSupportedTarget(target) {
  if (!SUPPORTED_TARGETS.has(target)) {
    throw new Error(`unsupported_skill_target: ${target}`);
  }
}

function absolutePath(path, workDir) {
  if (isAbsolute(path)) {
    return path;
  }
...
export function makeSkillMarkdown({
  target = "generic",
...
Use \`${apiBaseUrl}\` as the default \`<apiBaseUrl>\` unless the user or workspace config provides another registry URL.
...
export async function writeSkill({
  ...
  const outputPath = output
    ? absolutePath(output, baseDir)
    : join(baseDir, ".agentcart", "skills", `agentcart-${target}.md`);
  const markdown = makeSkillMarkdown({ target, apiBaseUrl });
...
export async function readCanonicalSkill() {
  return readFile(CANONICAL_SKILL_PATH, "utf8");
}
